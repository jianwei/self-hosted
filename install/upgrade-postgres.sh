...
if [[ -n "$($CONTAINER_ENGINE volume ls -q --filter name=sentry-postgres)" && "$($CONTAINER_ENGINE run --rm -v sentry-postgres:/db registry.cn-hangzhou.aliyuncs.com/github88/busybox cat /db/PG_VERSION 2>/dev/null)" == "9.6" ]]; then
