...
CPU_AVAILABLE_IN_DOCKER=$($CONTAINER_ENGINE run --rm registry.cn-hangzhou.aliyuncs.com/github88/busybox nproc --all)
...
RAM_AVAILABLE_IN_DOCKER=$($CONTAINER_ENGINE run --rm registry.cn-hangzhou.aliyuncs.com/github88/busybox free -m 2>/dev/null | awk '/Mem/ {print $2}')
...
IS_KVM=$($CONTAINER_ENGINE run --rm registry.cn-hangzhou.aliyuncs.com/github88/busybox grep -c 'Common KVM processor' /proc/cpuinfo || :)
if [[ ! "$SKIP_SSE42_REQUIREMENTS" -eq 1 && "$IS_KVM" -eq 0 && "$DOCKER_ARCH" = "x86_64" ]]; then
  SUPPORTS_SSE42=$($CONTAINER_ENGINE run --rm registry.cn-hangzhou.aliyuncs.com/github88/busybox grep -c sse4_2 /proc/cpuinfo || :)
