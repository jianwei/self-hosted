...
sentry_cli="$CONTAINER_ENGINE run --rm -v /tmp:/work -e SENTRY_DSN=$SENTRY_DSN registry.cn-hangzhou.aliyuncs.com/github88/sentry-cli"
...
send_event() {
  ...
  local fingerprint_value=$(
    echo -n "$cmd_exit $error_msg $traceback" |
      $CONTAINER_ENGINE run -i --rm registry.cn-hangzhou.aliyuncs.com/github88/busybox md5sum |
      cut -d' ' -f1
  )
...
if [ "$REPORT_SELF_HOSTED_ISSUES" == 1 ]; then
  if ! $CONTAINER_ENGINE pull registry.cn-hangzhou.aliyuncs.com/github88/sentry-cli:latest; then
